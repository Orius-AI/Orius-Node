/**
 * Orius Network - Chrome Extension
 * Compute Node Controller with Device-Bound Wallet Security
 * Developed by Orius Team
 */

const isPreviewMode = typeof chrome === 'undefined' || !chrome.storage;
...
async function init() {
  ...
  const isPreview = !window.chrome?.storage;
  const loadTime = isPreview ? 100 : CONFIG.loadingDuration;
  
  await initializeComputeEngine();
...
async function initializeComputeEngine() {
  try {
    if (typeof OriusComputeEngine !== 'undefined') {
      computeEngine = new OriusComputeEngine();
      await computeEngine.initialize();
      console.log('Compute engine initialized:', computeEngine.capabilities);
      
      if (deviceId) {
        taskClient = new OriusTaskClient(CONFIG.apiUrl, deviceId);
        
        taskClient.on('taskStart', (task) => {
          addRealActivityLog({
            type: 'task_start',
        ...
        taskClient.on('taskComplete', (task, result) => {
        ...
        taskClient.on('creditsEarned', (credits, task) => {
          console.log(`Earned ${credits} credits from task ${task.task_type}`);
        });
        
        taskClient.on('error', (error) => {
          console.error('Task client error:', error);
        });
      }
    }
  } catch (error) {
    console.log('Compute engine init skipped:', error.message);
  }
}

function addRealActivityLog(data) {
  const now = new Date();
...
function renderRealActivityLog() {
  ...
  el.activityLog.innerHTML = activityLogs.map(log => {
    const icon = log.type === 'task_complete' ? '&#10003;' : '&#9679;';
    const color = log.verified ? '#22c55e' : '#3b82f6';
    return `
      <div class="log-entry">
        <span class="log-task" style="color: ${color}">${icon} ${log.taskType}</span>
        <span class="log-result">${log.credits > 0 ? '+' + log.credits.toFixed(2) + ' credits' : 'processing...'}</span>
  ...
  try {
    if (typeof chrome !== 'undefined' && chrome.system?.cpu) {
      chrome.system.cpu.getInfo(info => {
        el.cpuCores.textContent = `${info.numOfProcessors} cores`;
      });
      chrome.system.memory.getInfo(info => {
        const gb = (info.capacity / (1024 * 1024 * 1024)).toFixed(1);
        el.memoryInfo.textContent = `${gb} GB`;
      });
    } else {
...
function notifyBackground(type) {
  if (typeof chrome !== 'undefined' && chrome.runtime?.sendMessage) {
    chrome.runtime.sendMessage({ type, deviceId }, (response) => {
...
async function startNode() {
  ...
  if (taskClient && computeEngine && state.isRegistered) {
    try {
      await taskClient.initialize(computeEngine);
      taskClient.start();
      console.log('Real compute tasks started');
    } catch (error) {
      console.log('Task client start failed:', error.message);
      startComputeWorker();
    }
  } else {
    startComputeWorker();
...
function startComputeWorker() {
  if (taskClient && computeEngine) {
    console.log('Using real compute tasks instead of simulation worker');
    return;
  }
